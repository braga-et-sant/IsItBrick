#https://github.com/flipflipshift/BasicYGOSim/blob/main/YugiohSim.py
...
def is_one_valid_draw(hand, extras, possibilities, can_extrav, can_desires, can_upstart, can_prosperity, can_duality):
    ...
    if can_duality and "Duality" in hand:
        for i in range(0, 3):
            ...
            temp_hand.append(temp_extras[i])
            del temp_extras[0:3]
            if is_one_valid_draw(temp_hand, temp_extras, possibilities, False, can_desires, can_upstart, can_prosperity,
                                 False):
                return True
    return False

def run_sim(deck_size, hand_size, input_cards_here, input_possibilities_here, num_trials):
    global card_hash
    card_hash = dict()
